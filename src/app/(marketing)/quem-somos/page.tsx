    ...
    <>
      ...
      >
        <div className="flex items-stretch w-full">
          <div className="w-2/3 max-w-full flex-shrink-0 flex flex-col gap-[60px] self-center">
            <p className="text-zinc-500 text-lg">Olá, somos a equipe do LocalizAçaí, um grupo de acadêmicos apaixonados por açaí, que encontrou em uma observação de campo a oportunidade de ajudar a população belenense e região através da democratização do acesso a informação, facilitando sua busca por um açaí de qualidade e devidamente manipulado, segundo as normas estabelecidas pela vigilância sanitária.</p>
            <p className="text-zinc-500 text-lg">Foi devido ao aumento de casos de Doença de Chagas no ano de 2026 que o grupo identificou que poderia criar uma forma contribuir com a população, utilizando de um sistema já existente que garante a segurança da produção do açaí, porém pouco conhecida.</p>
            <p className="text-zinc-500 text-lg">Partindo desse principio, nossa missão era desenvolver uma maneira fácil de encontrar estabelecimentos de venda de açaí que estejam adequados segundo as normas da vigilância sanitária.</p>
            <p className="text-zinc-500 text-lg">E assim nasceu o Localiz Açaí! Seu site de localização de pontos de venda de açaí devidamente certificados pela Região Metropolitana de Belém.</p>
            <p className="text-zinc-500 text-lg">Através do nosso site, nós queremos fornecer para todo mundo que queira aproveitar um bom açaí seguro praticidade e confiança ao escolher onde irá comprar, para desfrutar suas boas refeições.</p>
            <p className="text-zinc-500 text-lg">Este projeto foi idealizado por alunos da Universidade do Estado do Pará do curso de Bacharelado em Design, na disciplina de Projeto IV, a qual tem foco em design de interfaces, e aplicando suas particularidades projetuais para a concepção dessa interface a qual proporcionamos a experiência de acesso à informação de localização de pontos de açaí seguros.</p>
          </div>
          <div
            className="flex-1 scale-80 bg-contain bg-no-repeat bg-right-top mr-[20px] ml-auto"
        ...
        <div className="bg-[#f5f5f5]">
            ...
            <img src="/elements/decoration/quem_somos/homenzinho.webp" alt="" className="block h-72 w-auto ml-[80px]" />
      <section className="bg-background py-16 relative">
        ...
        <div className="overflow-x-auto scrollbar-hide w-full">
          <div className="flex flex-nowrap gap-6 px-16 w-max">
            {team.map((member, i) => (
              <div key={i} className="bg-[#f5f5f5] rounded-xl p-6 flex flex-col gap-4 w-[400px] flex-shrink-0">
                <div>
                  <h3 className="text-primary font-bold text-2xl leading-tight">{member.name}</h3>
                  <p className="text-secondary font-bold text-sm mt-2">{member.role}</p>
                </div>
                <div className="flex gap-2 items-start">
                  <span className="text-primary text-5xl leading-none select-none">&ldquo;</span>
